        ...
        timeout(time: 2, unit: 'HOURS') {
            dir('kura-bluetooth') {
                withMaven(jdk: 'temurin-jdk17-latest', maven: 'apache-maven-3.9.6', options: [artifactsPublisher(disabled: true)]) {
                    withCredentials([string(credentialsId: 'sonarcloud-token-kura-bluetooth', variable: 'SONARCLOUD_TOKEN')]) {
                        withSonarQubeEnv {
                            sh '''
                                mvn sonar:sonar \
                                    ...
                                    -Dsonar.projectKey=eclipse-kura_kura-bluetooth \
