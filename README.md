Eclipse Kura-Bluetooth
=============
Eclipse Kura™ Bluetooth addon

<p align="center">
<img src="https://eclipse.dev/kura/images/kura.png" alt="Kura™ logo" width="500"/>
</p>

Install
-------

Eclipse Kura-Bluetooth™ is compatible with Java 17.

### Target Installer
Eclipse Kura-Bluetooth™ provides an Eclipse Kura™ addon for managing Bluetooth communication.

### Documentation
Eclipse Kura-Bluetooth™ Documentation can be found at:

- [Kura Bluetooth APIs](https://eclipse-kura.github.io/kura/latest/java-application-development/how-to-use-bt-le-apis/)
- [Kura Beacon APIs](https://eclipse-kura.github.io/kura/latest/java-application-development/how-to-use-beacon-apis/)
- [Xdk Driver](https://eclipse-kura.github.io/kura/latest/connect-field-devices/xdk-driver/)
- [Eddystone Driver](https://eclipse-kura.github.io/kura/latest/connect-field-devices/edystone-driver/)
- [iBeacon Driver](https://eclipse-kura.github.io/kura/latest/connect-field-devices/ibeacon-driver/)
- [TI SensorTag Driver](https://eclipse-kura.github.io/kura/latest/connect-field-devices/sensortag-driver/)


Build
-----

### Prerequisites

In order to be able to build Eclipse Kura™ Bluetooth on your development machine, you need to have the following programs installed in your system:
* JDK 17
* Maven 3.9.x

<details>
<summary>

#### Installing Prerequisites in Mac OS 

</summary>

To install Java 17, download the JDK tar archive from the [Adoptium Project Repository](https://adoptium.net/en-GB/temurin/releases/?variant=openjdk8&jvmVariant=hotspot&version=17).

Once downloaded, copy the tar archive in `/Library/Java/JavaVirtualMachines/` and cd into it. Unpack the archive with the following command:

```bash
sudo tar -xzf <archive-name>.tar.gz
```

The tar archive can be deleted afterwards.

Depending on which terminal you are using, edit the profiles (.zshrc, .profile, .bash_profile) to contain:

```bash
export JAVA_HOME=/Library/Java/JavaVirtualMachines/<archive-name>/Contents/Home
```

Reload the terminal and run `java -version` to make sure it is installed correctly.

Using [Brew](https://brew.sh/) you can easily install Maven from the command line:

```bash
brew install maven@3.9
```
Run `mvn -version` to ensure that Maven has been added to the PATH. If Maven cannot be found, try running `brew link maven@3.9 --force` or manually add it to your path with:

```bash
export PATH="/usr/local/opt/maven@3.9/bin:$PATH"
```

</details>

<details>
<summary>

#### Installing Prerequisites in Linux

</summary>

For Java
```bash
sudo apt install openjdk-17-jdk
```
For Maven   

You can follow the tutorial from the official [Maven](http://maven.apache.org/install.html) site. Remember that you need to install the 3.9.x version.

</details>

### Build Eclipse Kura-Bluetooth™

Change to the new directory and clone the Eclipse Kura™ repo:

```bash
git clone https://github.com/eclipse-kura/kura-bluetooth.git
```

Move inside the newly created directory and build the project:

```bash
mvn clean install
```

> [!TIP]
You can skip tests by adding `-Dmaven.test.skip=true` in the command above.
